...
class BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  void _handlePendingApproval({required BookingDetailResponse val, bool isAddExtraCharges = false, bool isEditExtraCharges = false}) async {
    appStore.setLoading(true);
  ...
  Widget buildTimeWidget({required BookingData bookingDetail}) {
    if (bookingDetail.bookingSlot == null) {
      return Text(formatDate(bookingDetail.date.validate(), format: DATE_FORMAT_3), style: boldTextStyle(size: 12));
    }
    return Text(
      TimeOfDay(
